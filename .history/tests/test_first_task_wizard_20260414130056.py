...
def test_first_run_completion_dialog_failure_exposes_diagnostics_action(qapp):
    ...
    diagnostics_button = dialog.findChild(type(dialog.closeButton()) if hasattr(dialog, "closeButton") else None, "first_run_diagnostics_button")
    if diagnostics_button is None:
